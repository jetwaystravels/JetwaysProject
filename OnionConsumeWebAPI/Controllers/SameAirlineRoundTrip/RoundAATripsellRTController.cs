...
{
    public class RoundAATripsellRTController : Controller
    {
        ...
        public IActionResult RoundAATripsellViewRT()
        {
        ...
        public async Task<IActionResult> PostReturnContactData(ContactModel contactobject, AddGSTInformation addGSTInformation)
        {
            ...
            return RedirectToAction("_RGetGstDetails", "RoundAATripsellRT", contactobject);
        }
        ...
        public async Task<IActionResult> _RGetGstDetails(ContactModel contactobject, AddGSTInformation addGSTInformation)
        {
            ...
            return RedirectToAction("RoundAATripsellViewRT", "RoundAATripsellRT");
        }
        [HttpPost]
                    ...
                    if (!string.IsNullOrEmpty(tokenview) && dataArray[i1].ToLower() == "airasia")
                    {
                        ...
                        for (int i = 0; i < passengerdetails.Count; i++)
                        {
                            ...
                            if (passengerdetails[i].passengertypecode != null)
                            {
                                ...
                                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                                HttpResponseMessage responsePassengers = await client.PutAsJsonAsync(AppUrlConstant.AirasiaAddPassenger + result, _PassengersModel);
                            ...
                            if (passengerdetails[j].passengertypecode == "INFT" || passengerdetails[j].passengertypecode == "INF")
                            {
                                for (int k = 0; k < infantcount; k++)
                                {
                                   string result = passengerdetails[k].passengerkey.Substring(0, passengerdetails[k].passengerkey.IndexOf('^'));
                                    ...
                                    HttpResponseMessage responsePassengers = await client.PostAsJsonAsync(AppUrlConstant.AirasiaAddPassenger + result + "/infant", _PassengersModel1);
                    ...
                    if (!string.IsNullOrEmpty(tokenview) && dataArray[i1].ToLower() == "akasaair")
                    {
                        ...
                        for (int i = 0; i < passengerdetails.Count; i++)
                        {
                            ...
                            if (passengerdetails[i].passengertypecode != null)
                            {
                                ...
                                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                                HttpResponseMessage responsePassengers = await client.PutAsJsonAsync(AppUrlConstant.AkasaAirPassengerDetails + resultAkasa, _PassengersModel);
                                if (responsePassengers.IsSuccessStatusCode)
                                {
                                    var _responsePassengers = responsePassengers.Content.ReadAsStringAsync().Result;
                                    Logs logs = new Logs();
                        ...
                        for (int i = 0; i < passengerdetails.Count; i++)
                        {
                            ...
                            if (passengerdetails[i].passengertypecode == "INFT" || passengerdetails[i].passengertypecode == "INF")
                            {
                                ...
                                for (int k = 0; k < infantcount; k++)
                                {
                                    ...
                                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                                    HttpResponseMessage responsePassengers = await client.PostAsJsonAsync(AppUrlConstant.AkasaAirPassengerDetails + resultAkasainft + "/infant", _PassengersModel1);
                                    if (responsePassengers.IsSuccessStatusCode)
                                    {
                                        var _responsePassengers = responsePassengers.Content.ReadAsStringAsync().Result;
                                        Logs logs = new Logs();
        ...
        public async Task<IActionResult> PostUnitkey(List<string> unitKey, List<string> ssrKey, List<string> BaggageSSrkey, List<string> FastfarwardAddon, List<string> PPBGAddon)
        {
            ...
            return RedirectToAction("RoundTripPaymentViewRT", "RoundTripPaymentGatewayRT");
        }
        public class Paxes
        {
